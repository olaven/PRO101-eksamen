...
<script>
    ...
    let getBysykkelJS = (type, callback) => {
        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                //var args = JSON.stringify(this.responseText);
                
                //var trimmedResponse = this.responseText.replace(/^\s*/,'').replace(/\s*$/,'').toLowerCase().;
                //var jsonObject = JSON.parse(trimmedResponse);
    ...
    var markersTest =
        '{"result":true,"count":1}'
    var toJson = JSON.parse(markersTest);
    ...
    function initMap() {
        ...
        getBysykkelJS("stations", (response) => {
            ...
            for (var i = 0; i < empty.length; i++) {
                var data = empty[i];
                console.log(empty[i]["longitude"]);
                var myLatlng = new google.maps.LatLng(data.longitude, data.latitude);
                var markerBysykkel = new google.maps.Marker({
                    position: myLatlng,
                    map: map,
                    title: data.title
                });
