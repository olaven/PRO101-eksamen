...
// TUT Customization - Start 2
    ...
    $.redux.get_container_value = function( id ) {
        ...
        if( $( 'body' ).hasClass( 'wp-customizer' ) ) {
            var value = $( '#' + id + '.checkbox' ).attr( 'value' ); // ThinkUpThemes customization - Only works for checkboxes & switches with 1 / 0 values
        } else {
    ...
    $.redux.required_recursive_hide = function( id ) {
        if( $( 'body' ).hasClass( 'wp-customizer' ) ) {
            var toFade = $( '#' + redux.args.opt_name + '-' + id );
        } else {
        ...
        if ( redux.required_child.hasOwnProperty( id ) ) {
            $.each(
                redux.required_child[id], function( i, parentData ) {
                    if( $( 'body' ).hasClass( 'wp-customizer' ) ) {
                        var current = $( '#' + redux.args.opt_name + '-' + parentData.parent );
                    } else {
                        var current = $( '#' + redux.args.opt_name + '-' + parentData.parent ).parents( 'tr:first' );
                    }
...
// TUT Customization - Disable Redux Adverts in Customizer - START
...
//        }
//    );
// TUT Customization - Disable Redux Adverts in Customizer - END
})( jQuery );
